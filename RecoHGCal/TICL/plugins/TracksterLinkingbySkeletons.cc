    ...
    for (int eta_i = search_box[0]; eta_i <= search_box[1]; ++eta_i) {
      for (int phi_i = search_box[2]; phi_i <= search_box[3]; ++phi_i) {
        ...
        for (auto n : neighbours) {
          ...
          auto const deltaR_etaphi = sqrt((trackster.barycenter().eta() - tracksterOut.barycenter().eta()) *
                                              (trackster.barycenter().eta() - tracksterOut.barycenter().eta()) +
                                          (trackster.barycenter().phi() - tracksterOut.barycenter().phi()) *
                                              (trackster.barycenter().phi() - tracksterOut.barycenter().phi()));
          auto deltaphi = reco::deltaPhi(trackster.barycenter().phi(), tracksterOut.barycenter().phi());
